...
import com.mazenet.mani.gurugubera.Adapters.ShowAuctionReportList
...
class AuctionReport : BaseFragment() {

    val customerlist = ArrayList<AuctionReportModel>()
    val customerlist_showing = ArrayList<AuctionReportModel>()
    lateinit var recycler_collreports: RecyclerView
    lateinit var collReportAdapte: ShowAuctionReportList
    ...
    override fun onCreateView(
        ...
        (activity as HomeActivity).setActionBarTitle("Auction Report")
        ...
        txt_cr_col2.setText(resources.getString(R.string.groupname))
        txt_cr_col3.setText(resources.getString(R.string.treleaseamount))
        txt_disp_total.setText(resources.getString(R.string.tot_release))
            ...
            override fun onTextChanged(
                ...
                if (text == "" || text == null) {
                    ...
                    if (customerlist_showing.size > 0) {
                        ...
                        collReportAdapte =
                            ShowAuctionReportList(customerlist_showing, object : AdapterClickListener {
                                override fun onPositionClicked(view: View, position: Int) {
                                    if (view.getTag().equals("listitem")) {
                                        val bundle = Bundle()
                                        bundle.putString("groupid", customerlist_showing.get(position).getGroupId())
                                        bundle.putString("custid",customerlist_showing.get(position).getCustomerId())
                                        doFragmentTransactionWithBundle(AuctionReportDetail(), "viewpreview", true, bundle)
...
//                mobile = mobile.toLowerCase(Locale.getDefault())
        ...
        if (customerlist_showing.size > 0) {
            recycler_collreports.setVisibility(View.VISIBLE)
            collReportAdapte = ShowAuctionReportList(customerlist_showing, object : AdapterClickListener {
                override fun onPositionClicked(view: View, position: Int) {
                    if (view.getTag().equals("listitem")) {
                        val bundle = Bundle()
                        bundle.putString("groupid", customerlist_showing.get(position).getGroupId())
    ...
    fun get_Collectionlist(
        ...
        if (checkForInternet()) {
            showProgressDialog()
            println("fetching")
            var resultlist = java.util.ArrayList<AuctionReportModel>()
            ...
            val LeadListRequest = getleadslist.get_auction_reports(loginparameters)
            LeadListRequest.enqueue(object : Callback<ArrayList<AuctionReportModel>> {

                override fun onFailure(call: Call<ArrayList<AuctionReportModel>>, t: Throwable) {
                    hideProgressDialog()
                    t.printStackTrace()
                ...
                override fun onResponse(
                    call: Call<ArrayList<AuctionReportModel>>,
                    response: Response<ArrayList<AuctionReportModel>>
                ) {
                    hideProgressDialog()
                    when {
                        response.isSuccessful -> {
                            ...
                            when {
                                ...
                                else ->  recycler_collreports.visibility=View.GONE
                            }
    ...
    private fun integrateList(leadslist: ArrayList<AuctionReportModel>) {
        if (leadslist.size > 0) {
            customerlist.clear()
            customerlist_showing.clear()
            ...
            try {
                ...
                for (i in customerlist_showing) {
                    total += Constants.isEmtytoZero(i.getTotalReleaseAmount()!!).toInt()
                }
...
//                            col_area_layout.visibility = View.VISIBLE
    ...
    fun fromdate(textrange: TextView) {
        ...
        val fromDatePickerDialog = context?.let {
            DatePickerDialog(
                it,
